...
\section{insight}
A key insight to this thesis is that multiple applications will have the same definition of compliance.  Or, if not the same exact definition of compliance, there will be many shared logical building blocks.

\section{Example I'm not using but I didn't want to just delete}
\paragraph{Example}
...
$$C_{\text{1}} = (\text{sender}(\text{txn}) \notin A, B_{\text{current}})$$
$$C_{\text{2}} = (\text{sender}(\text{txn}) \notin A, B_{\text{previous}})$$

Compliance definition $R_1$ only requires that the sender isn't currently in sanction list $A$:
$$R_1 = C_1$$

Compliance definition $R_2$ requires that the sender wasn't in sanction list $A$ at the current block or the previous block, as the input $A$ can be updated by the regulation author: 

\reminder{Joss: $A$ should be on-chain, so $B_{\text{previous}}$ could be referencing a different $A$ than the $A$ at the current block.  This is something I need to expand upon.  Or maybe I just need to separate which inputs come from chain-data at the time of proof verification and which inputs come from the user}
