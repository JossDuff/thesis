...
We present a composable framework that resolves this conflict by enabling users to cryptographically prove compliance with any regulation without revealing private transaction data.  In our system, regulatory bodies publish compliance definitions as combinations of reusable logical primitives called constraints.  Applications select relevant compliance definitions based on their regulatory obligations.  Users generate zero-knowledge proofs demonstrating they meet these requirements, which are verified on-chain before transactions execute.  This provides proactive compliance: preventing non-compliant actions rather than detecting them afterward, all while maintaining strong privacy guarantees.
...
Centralized exchanges can also be used as custodial mixers.
...
\section{Desired properties of privacy preserving compliance}
...
Any compliant privacy solution that gives privileges to decrypt private user data has deanonymization.  The ability for a third party to reveal a user's private details is fundamentally weaker privacy.  We can guarantee that private transactions can only be revealed by the user, and do so without compromising compliance.  We also provide stronger compliance guarantees than viewing key compliance.  Viewing keys are used for retroactive compliance checks, and our framework has proactive compliance: non-compliant actors are stopped before making their transaction rather than after.
...
An on-chain privacy system where a third party also knows secret user data is as ``private" as a traditional exchange or bank.  In order for a on-chain privacy system to provide unique value to users, it must not allow for deanonymization.
...
\paragraph{Secret inputs:} Allow for arguments in the compliance evaluation that are only known to the subject.  This is trivial in most solutions that utilize ZK proofs for compliance verification, as ZK proofs inherently allow secret inputs.  However this is difficult to achieve when the compliance evaluation is done on a remote server.
...
\hline
Nightfall~\cite{ey2025blockchain} &\xmark &\xmark &\xmark &\xmark &\xmark &\xmark &\xmark &\xmark \\
\hline
\rowcolor{blue!10}
...
The cost to verify a compliance proof on-chain does not grow with the size of the compliance definition.  This is inherited from using zkSNARKs as compliance proofs.  The circuit can be very computationally intensive and have pages of criteria in its natural language form, but it will still cost the same amount as a circuit with a single operation. 
...
\begin{itemize}
    ...
    \item Constraint update (ex: add or remove a constraint from the definition) modifies the circuit
    \begin{itemize}
        \item When using PLONK, circuit modifications are feasible, as the trusted setup can be reused.
        \item A constraint update also requires regulators to publicly re-publish and sign the new compliance definition.
    \end{itemize}
\end{itemize}
...
Chainalysis, a leading blockchain analytics company used by government bodies, adds addresses to their sanctions lists every 30 days~\cite{chainalysissanctionsoracle}.  If regulators publish a new verifier contract each time a sanction list is updated for a compliance definition, the applications using that compliance definition must manually update their smart contracts to reflect the new verifier contract address.   Using 30 days as a benchmark as to how often compliance definitions might have to change, it is infeasible to require all applications to make smart-contract changes this frequently because the applications expect this framework to be an easy to use compliance tool.  We don't want to introduce the potential for a mistake as small as an application updating a day late to result in legal action.  A priority of this framework is to provide applications with a strong guarantee of compliance and minimal overhead to the application.
...
\begin{itemize}
    ...
    \item Benchmark prototype constraints
...
\begin{codebox}
\Procname{Current:}
\li $\exists \, \text{t} \in \text{txn\_history}(\text{sender}(\text{txn}), [B_0, B_{\text{current}}]) : \text{recipient}(\text{t}) = p_{\text{protocol}}$
