...
An on-chain privacy system where a 3rd party also knows secret user data is as ``private" as a traditional exchange or bank.  In order for a on-chain privacy system to provide unique value to users, it must not allow for deanonymization.
...
\hline
Aleo~\cite{aleo2025homepage} (ZEXE~\cite{ZEXE}) &\checkmark &\xmark &\xmark &\xmark &\qmark &\xmark &\xmark &\checkmark \\
\hline
...
\hline
Nightfall~\cite{ey2025blockchain} &\qmark &\qmark &\qmark &\qmark &\qmark &\qmark &\xmark &\xmark \\
\hline
Starlight~\cite{ey2025blockchain} &\qmark &\qmark &\qmark &\qmark &\qmark &\qmark &\xmark &\xmark \\
\hline
\rowcolor{blue!10}
...
\reminder{Joss: I asked Paul where I could find some technical documentation on nightfall and starlight.  So far all I've found is marketing material}
...
\li \>\>$\id{metadataHash}$ (IPFS hash of compliance circuit)
\li
...
Compliance definitions can be published before $t_\text{start}$ time.  If it is published far enough ahead of $t_\text{start}$, it is likely that the update transaction will be finalized all chains by $t_\text{start}$ when the update goes into effect.  This is a natural consequence of making time a first-class parameter in compliance definitions.
