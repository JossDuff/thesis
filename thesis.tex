...
A framework for on-off ramping privacy preserving L2s app rollups to and from L1.  Look at Hibachi, a privacy preserving CLOB app rollup which settles to Celestia.  My thesis is that app rollups and ephemeral rollups are the true usecase of L2s.  EVM L2s were exploratory and a proof of concept.  We're reaching the stage of actually utilizing off-chain properties (not public by default, skip consensus, faster compute) while periodically gaining security guarentees of the underlying DA layer.  This implies that rollups should use the DA layer with the best security.  If security is measured by total cost of attack, then Ethereum is the most secure network. 
