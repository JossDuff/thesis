...
A goal of this framework is to be flexible enough to accommodate compliance definitions over all aspects of a transaction: the payer, the payee, the content of the transaction, and even the actors' transaction histories.  Flexibility will result in the ability to express any definition of compliance.
...
In this prototype, applications integrate this framework by requiring proofs of compliance rules for specified entrypoint functions.  Applications choose compliance definitions based on the applications regulatory goals.  
...
The compliance rule requires both sender and recipient to satisfy the non-membership constraint:
...
To transform written law into a ZK proof there needs to be a human readable intermediary syntax.  The constraint system introduced in \ref{sec:constraints} can be extended into a novel DSL user by regulatory bodies to define compliance.  If this novel DSL is sufficiently advanced, it will be able to extract constraints proven in proof A for the generation of proof B to save on proving costs.
